...
import { ChevronUp, Sword, Target, Trash2, Flame, Plus, Zap, Eye } from 'lucide-react';
import { db, appId } from '../lib/firebase';
import { calculateCasterLevel, formatModifier, getModifier, parseDamage } from '../utils/stats';
import { CharacterData, Weapon, Spell, Stats } from '../types';
import { RACES, CLASSES, BACKGROUNDS_DATA, SPELL_SLOT_TABLE, STAT_LABELS, STAT_ORDER, SPELL_DB } from '../constants/gameData';
...
export const CharacterSheet = ({ user, roomCode, targetUid, isDM }: CharacterSheetProps) => {
  ...
  const updateChar = async (newData: CharacterData) => {
    if (!canEdit || !targetUid || !roomCode) return;
    setCharData(newData);
  ...
  const performAttack = async (weapon: Weapon) => {
      if (!charData || !targetUid) return;
      ...
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', `room_${roomCode}_rolls`), logData as any);
  };
